...
For boootstrap estimate $\hat\theta^*=S(P^*)$ , ideal bootstrap estimate of variance of $\hat\theta$ can be written as $Var(S(P^*))$ where $P^*$ denotes the bootstrap resampling vector with the form $\frac{1}{n}(N_1, \cdots, N_n)$ and $nP^*\sim Multinomial(n, P_0)$  
...
$$
Var(S(P^*))=\frac{n-1}{n}\cdot \frac{\sum_{i=1}^n (x_i-\overline{x})^2}{n(n-1)}=\frac{1}{n^2}\sum_{i=1}^n (x_i-\overline{x})^2
$$
